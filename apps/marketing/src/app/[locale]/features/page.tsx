...
export function generateMetadata({ params }: FeaturesPageProps): Metadata {
  const locale = isSupportedLocale(params.locale) ? params.locale : DEFAULT_LOCALE;
  const i18n = getServerI18n(locale);
  return buildPageMetadata({
    title: i18n.t('pageTitle.features'),
    description: i18n.t('pageDescription.features'),
    locale,
    path: '/features',
  });
}

export default function FeaturesPage({ params }: FeaturesPageProps): ReactNode {
  const locale = isSupportedLocale(params.locale) ? params.locale : DEFAULT_LOCALE;
  const i18n = getServerI18n(locale);
