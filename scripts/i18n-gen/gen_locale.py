...
def emit_value(value, indent, translations, full_key, is_critical, missing):
    ...
    Returns (value_str, comment) where comment is an optional trailing
    comment (without the leading `//`). The caller is responsible for
    placing the comma."""
    typ, v = value
    if typ == 'string':
        if full_key in translations:
            return ("'" + escape_ts_string(translations[full_key]) + "'", None)
        elif is_critical:
        ...
        if is_plural:
            ...
            for sk, (st, sv) in v.items():
                ...
                val_str, comment = emit_value(('string', sv), indent + '  ', translations, sub_full_key, is_critical, missing)
                if comment:
                    lines.append(f"  {indent}{sk}: {val_str},  // {comment}")
                else:
                    lines.append(f"  {indent}{sk}: {val_str},")
            ...
            for sk, sv in v.items():
                sub_full_key = f"{full_key}.{sk}" if full_key else sk
                val_str, comment = emit_value(sv, indent + '  ', translations, sub_full_key, is_critical, missing)
                if comment:
                    lines.append(f"  {indent}{sk}: {val_str},  // {comment}")
                else:
                    lines.append(f"  {indent}{sk}: {val_str},")
...
def emit_namespace_file(namespace, subdir, translations, locale, is_critical, needs_review):
    ...
    for key, value in keys.items():
        ...
        val_str, comment = emit_value(value, '  ', translations, translation_key, is_critical, missing)
        if comment:
            body_lines.append(f"  {escape_key(key)}: {val_str},  // {comment}")
        else:
            body_lines.append(f"  {escape_key(key)}: {val_str},")
    body = '\n'.join(body_lines)
