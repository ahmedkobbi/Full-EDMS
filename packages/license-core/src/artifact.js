...
exports.LICENSE_ARTIFACT_VERSION = void 0;
exports.buildLicenseArtifact = buildLicenseArtifact;
exports.verifyLicenseArtifact = verifyLicenseArtifact;
 ...
 */
function buildLicenseArtifact(payload, privateKeyPem, kid, alg) {
    if (typeof kid !== 'string' || kid.length === 0) {
        throw new Error('buildLicenseArtifact: kid must be a non-empty string');
    }
    const sig = (0, sign_js_1.signPayload)(payload, privateKeyPem, alg);
    return {
        v: exports.LICENSE_ARTIFACT_VERSION,
        type: 'sedms.license',
        alg: (0, keys_js_1.algToSpecAlg)(alg),
        kid,
        payload,
        sig,
    };
 ...
 *   on any failure.
 */
function verifyLicenseArtifact(artifact, publicKeyPem) {
    // Structural sanity checks (fail closed).
    if (artifact == null || typeof artifact !== 'object') {
        return { ok: false, reason: 'artifact is not an object' };
    }
    if (artifact.v !== exports.LICENSE_ARTIFACT_VERSION) {
    ...
    let alg;
    try {
        alg = (0, keys_js_1.specAlgToAlg)(artifact.alg);
    }
    catch (e) {
        return {
            ok: false,
    ...
    const ok = (0, sign_js_1.verifySignature)(artifact.payload, artifact.sig, publicKeyPem, alg);
    if (!ok) {
