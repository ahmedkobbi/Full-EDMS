...
/**
 * User subset accepted by `buildContextEnvelope`. Carries ONLY the fields
 * the envelope is allowed to surface — secrets are not even part of the
 * input type, so callers cannot accidentally pass them.
 */
export interface ContextEnvelopeUser {
  readonly id: UserId;
  readonly roles: readonly string[];
  /**
   * Coarse permission summary (e.g. `['documents:read', 'workflows:read']`).
   * NOT the full permission list — just enough for the planner to decide
   * which tool suggestions are relevant.
   */
...
/** Tenant subset accepted by `buildContextEnvelope`. */
export interface ContextEnvelopeTenant {
  readonly id: TenantId;
  readonly currentRoute: string;
}

/**
 * License subset accepted by `buildContextEnvelope`. Carries ONLY the
 * fields the envelope is allowed to surface — the signing key material,
 * heartbeat secrets, and CRL fetch URLs are NOT included.
 */
export interface ContextEnvelopeLicense {
  readonly licensedModules: readonly EntitlementModule[];
}

/** Tour context entry the builder accepts (already-projected, no PII). */
export interface ContextEnvelopeTourEntry {
  readonly tourId: TourDefinitionId;
  readonly status: string;
}

/** Input bundle for `buildContextEnvelope`. */
export interface BuildContextEnvelopeInput {
  readonly user: ContextEnvelopeUser;
  readonly tenant: ContextEnvelopeTenant;
  readonly license: ContextEnvelopeLicense;
  readonly theme: 'light' | 'dark';
  readonly requestId: UUID;
  readonly tourContext?: readonly ContextEnvelopeTourEntry[];
  /**
   * Optional timestamp generator (default: `new Date().toISOString()`).
   * Override in tests for determinism. Note: the envelope itself does NOT
   * carry a timestamp field — this is only used by `buildContextEnvelope`
   * for any internal bookkeeping that needs one. Currently unused but
   * reserved for future `envelope.issuedAt` fields.
   */
 ...
 */
export function buildContextEnvelope(input: BuildContextEnvelopeInput): AiContextEnvelope {
  ...
  return {
    ...
    theme: input.theme,
    tourContext: Object.freeze(
      (input.tourContext ?? []).map((t) => ({
        tourId: t.tourId,
        status: t.status,
      })),
    ),
