 ...
 */
export const PROMPT_INJECTION_MAX_INPUT_LENGTH = 16_384;

/** Category bucket each pattern maps to. */
type InjectionCategory = PromptInjectionDetection['category'];

/** Operation mode for `detectPromptInjection`. */
export type PromptInjectionMode = 'block' | 'audit';
...
export function detectPromptInjection(
  ...
  const haystack =
    input.length > PROMPT_INJECTION_MAX_INPUT_LENGTH
      ? `${input.slice(0, PROMPT_INJECTION_MAX_INPUT_LENGTH)}…`
      : input;
